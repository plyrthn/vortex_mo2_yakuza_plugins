...
const GAME_ID = 'likeadragoninfinitewealth';
...
function main(context) {
    ...
    context.registerInstaller(
        'likeadragoninfinitewealth-mod-installer',
        25,
        testMod,
    ...
    return new Promise((resolve, reject) => {
        function fetch(url, isRedirect) {
            ...
            mod.get(url.startsWith('http') ? url : {
                ...
                path: url,
                headers: {'User-Agent': 'vortex-likeadragoninfinitewealth'},
            }, (res) => {
...
function downloadFile(url, dest) {
    return new Promise((resolve, reject) => {
        function fetch(u) {
            https.get(u, {headers: {'User-Agent': 'vortex-likeadragoninfinitewealth'}}, (res) => {
                if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
            ...
            api.events.emit('start-install-download', dlId, true, (err, modId) => {
                ...
                util.batchDispatch(api.store, [
                    ...
                    actions.setModAttributes(GAME_ID, modId, {
                        ...
                        description: 'Modding requirement for Like a Dragon: Infinite Wealth - keep enabled.',
                    }),
                ]);
                resolve(modId);
            });
...
async function downloadSRMM(api) {
    api.sendNotification({
        id: 'likeadragoninfinitewealth-srmm-install',
        type: 'activity',
    ...
    try {
        ...
        api.dismissNotification('likeadragoninfinitewealth-srmm-install');
    }
}
