...
const MOD_TYPE_RMM = 'yakuzalikeadragon-rmm-modmanager-modtype';
...
function main(context) {
    ...
    context.registerInstaller(
        'yakuzalikeadragon-rmm-modmanager-installer',
        20,
        testRMM,
    ...
    context.registerInstaller(
        'yakuzalikeadragon-mod-installer',
        25,
        testMod,
    ...
    return new Promise((resolve, reject) => {
        function fetch(url, isRedirect) {
            ...
            mod.get(url.startsWith('http') ? url : {
                ...
                path: url,
                headers: {'User-Agent': 'vortex-yakuzalikeadragon'},
            }, (res) => {
...
function downloadFile(url, dest) {
    return new Promise((resolve, reject) => {
        function fetch(u) {
            https.get(u, {headers: {'User-Agent': 'vortex-yakuzalikeadragon'}}, (res) => {
                if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
            ...
            api.events.emit('start-install-download', dlId, true, (err, modId) => {
                ...
                util.batchDispatch(api.store, [
                    ...
                    actions.setModAttributes(GAME_ID, modId, {
                        ...
                        description: 'Modding requirement for Yakuza: Like a Dragon - keep enabled.',
                    }),
                ]);
                resolve(modId);
            });
...
async function downloadSRMM(api) {
    api.sendNotification({
        id: 'yakuzalikeadragon-srmm-install',
        type: 'activity',
    ...
    try {
        ...
        api.dismissNotification('yakuzalikeadragon-srmm-install');
    }
}
