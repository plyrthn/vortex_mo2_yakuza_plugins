...
const GAME_ID = 'lostjudgment';
const STEAMAPP_ID = '2058190';
...
const GAME_EXE = path.join('runtime', 'media', 'LostJudgment.exe');

const MOD_TYPE_RMM = 'lostjudgment-rmm-modmanager-modtype';
...
function main(context) {
    ...
    context.registerInstaller(
        'lostjudgment-rmm-modmanager-installer',
        20,
        testRMM,
    ...
    return new Promise((resolve, reject) => {
        function fetch(url, isRedirect) {
            ...
            mod.get(url.startsWith('http') ? url : {
                ...
                path: url,
                headers: {'User-Agent': 'vortex-lostjudgment'},
            }, (res) => {
...
function downloadFile(url, dest) {
    return new Promise((resolve, reject) => {
        function fetch(u) {
            https.get(u, {headers: {'User-Agent': 'vortex-lostjudgment'}}, (res) => {
                if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
            ...
            api.events.emit('start-install-download', dlId, true, (err, modId) => {
                ...
                util.batchDispatch(api.store, [
                    ...
                    actions.setModAttributes(GAME_ID, modId, {
                        ...
                        description: 'Modding requirement for Lost Judgment - keep enabled.',
                    }),
                ]);
                resolve(modId);
            });
...
async function downloadSRMM(api) {
    ...
    try {
        ...
        api.dismissNotification('lostjudgment-srmm-install');
    }
}
