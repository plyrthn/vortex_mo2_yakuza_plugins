...
const GAME_ID = 'likeadragongaiden';
const STEAMAPP_ID = '2375550';
...
function main(context) {
    ...
    context.registerInstaller(
        'likeadragongaiden-mod-installer',
        25,
        testMod,
    ...
    return new Promise((resolve, reject) => {
        function fetch(url, isRedirect) {
            ...
            mod.get(url.startsWith('http') ? url : {
                ...
                path: url,
                headers: {'User-Agent': 'vortex-likeadragongaiden'},
            }, (res) => {
...
function downloadFile(url, dest) {
    return new Promise((resolve, reject) => {
        function fetch(u) {
            https.get(u, {headers: {'User-Agent': 'vortex-likeadragongaiden'}}, (res) => {
                if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
            ...
            api.events.emit('start-install-download', dlId, true, (err, modId) => {
                ...
                util.batchDispatch(api.store, [
                    ...
                    actions.setModAttributes(GAME_ID, modId, {
                        ...
                        description: 'Modding requirement for Like a Dragon Gaiden - keep enabled.',
                    }),
                ]);
                resolve(modId);
            });
...
async function downloadSRMM(api) {
    api.sendNotification({
        id: 'likeadragongaiden-srmm-install',
        type: 'activity',
    ...
    try {
        ...
        api.dismissNotification('likeadragongaiden-srmm-install');
    }
}
